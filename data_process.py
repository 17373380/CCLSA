...
class SentenceDataset(Dataset):
    def __init__(self, opt, data_path, text_tokenizer, photo_path, image_transforms, data_type, data_translation_path=None, data_caption_path=None, image_coordinate=None):
        ...
        #提取caption文件
        file_read = open(data_caption_path, 'r', encoding='utf-8')
        file_content = json.load(file_read)
        file_read.close()
        self.data_caption_id_to_text_dict = {data['id']: data['caption'] for data in file_content}
        ...
        if opt.text_model == 'bert-base':
            ...
            self.text_caption_id_to_token_list = {index: text_tokenizer.tokenize('[CLS]' + text + '[SEP]') for
                                                      index, text in self.data_caption_id_to_text_dict.items()}
        ...
        # 限制长度
        self.text_caption_id_to_token_list = {
            index: text_token if len(text_token) < opt.word_length else text_token[0:opt.word_length] for
            index, text_token in self.text_caption_id_to_token_list.items()}
        # token放进去生成id
        self.text_caption_to_id = {index: text_tokenizer.convert_tokens_to_ids(text_token) for index, text_token in
                                       self.text_caption_id_to_token_list.items()}
    ...
    def __getitem__(self, index):
        ...
        return self.text_to_id[index], image_origin, self.label_list[index], self.text_translation_to_id[self.data_id_list[index]], self.text_caption_to_id[self.data_id_list[index]], image_augment
...
class Collate():
    ...
    def _collate(self, batch_data):
        ...
        text_caption_to_id = [torch.LongTensor(b[4]) for b in batch_data]
        image_augment = torch.FloatTensor([np.array(b[5]) for b in batch_data])
        ...
        data_caption_length = torch.LongTensor([text.size(0) for text in text_caption_to_id])
        ...
        max_caption_length = max(data_caption_length)
        if max_caption_length < self.min_length:
            text_caption_to_id[0] = torch.cat((text_caption_to_id[0], torch.LongTensor(
                [0] * (self.min_length - text_caption_to_id[0].size(0)))))
        ...
        text_caption_to_id = run_utils.pad_sequence(text_caption_to_id, batch_first=True, padding_value=0)
        ...
        # 要生成两个掩码，一个是放在文字的位置，一个是放在图像的位置
        caption_mask = []
        caption_image_mask = []
        for length in data_caption_length:
            text_mask_cell = [1] * length
            text_mask_cell.extend([0] * (max_caption_length - length))
            caption_mask.append(text_mask_cell[:])

            text_mask_cell.extend([1] * self.image_mask_num)
            caption_image_mask.append(text_mask_cell[:])

        text_caption_mask = []
        for element1, element2 in zip(tran_bert_attention_mask, caption_mask):
            result = element1 + element2
            text_caption_mask.append(result)
        ...
        return (text_to_id, torch.LongTensor(bert_attention_mask), image_origin, torch.LongTensor(text_image_mask),
                label, text_translation_to_id, torch.LongTensor(tran_bert_attention_mask), text_caption_to_id,
                torch.LongTensor(caption_mask), image_augment,
                torch.LongTensor(tran_text_image_mask), torch.LongTensor(caption_image_mask),
                torch.LongTensor(text_caption_mask), target_labels)
...
def data_process(opt, data_path, text_tokenizer, photo_path, data_type, data_translation_path=None, data_caption_path=None, image_coordinate=None):
    ...
    dataset = SentenceDataset(opt, data_path, text_tokenizer, photo_path, transform_train if data_type == 1 else transform_test_dev, data_type,
                              data_translation_path=data_translation_path, data_caption_path=data_caption_path, image_coordinate=image_coordinate)
